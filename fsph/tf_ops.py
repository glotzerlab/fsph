...
def _spherical_harmonic_series_grad(op, grad):
    ...
    # result::(..., 2, 1)
    result = (tf.linalg.matmul(tf.conj(input_grad), grad, transpose_a=True) +
              tf.linalg.matmul(input_grad, tf.conj(grad), transpose_a=True))
    # result -> (..., 2)
    result = tf.real(tf.squeeze(result, -1))*0.5
